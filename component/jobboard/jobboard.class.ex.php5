    ...
    $sHTML = $oHTML->getBlocStart('',array('class'=>'jobSearchContainer '.$sClass));
      $sHTML.= $oHTML->getBlocStart('',array('class'=>'jobSearchContainerInner'.$sFullSearchClass));
        ...
        $sHTML.= $oHTML->getBlocStart('',array('class'=>'newSearchDiv '));
          ...
          $sHTML.= $oHTML->getLink($this->casText['TALENT_CLOSE_SEARCH'].' '.$sPicture, 'javascript:;', array('onclick'=> '$(this).closest(\'.jobSearchContainerInner\').fadeOut(\'fast\', function(){ $(\'.quickSearch\').fadeIn(); });', 'style' => 'line-height: 20px; position: relative; padding-right: 25px; color: #555;'));
          $sHTML.= $oHTML->getBlocEnd();
