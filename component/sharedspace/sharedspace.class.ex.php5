...
{
  //to move in a db table at some point
  private $casDocumentType = array();
      ...
      case CONST_SS_TYPE_DOCUMENT:
        ...
        switch($this->csAction)
        {
          case CONST_ACTION_DELETE:
            exit(json_encode($this->_getDocumentDelete($this->cnPk)));
            break;
        }
        break;
    }
  ...
  private function _getCronDocument()
  {
    ...
    //get the list of all users (all cause we need the name of creators)
    $asAllUsers = $oLogin->getUserList();
    $asActiveUsers = array();
    foreach($asAllUsers as $asUserData)
    {
      if((int)$asUserData['status'] == 1 && (int)$asUserData['is_admin'] == 0)
        $asActiveUsers[$asUserData['loginpk']] = $asUserData;
    }

    /*dump($asAllUsers); echo '<hr />';
    dump($asActiveUsers);*/
    ...
    if(!empty($sMailComponent))
    {
      ...
      foreach($asToNotify as $nLoginfk => $anDocumentToNotify)
      {
        ...
        {
          $sContent = '<h3>Dear '.$oLogin->getUserNameFromData($asActiveUsers[$nLoginfk]).',</h3><br /><br />';
          ...
          if(CONST_DEV_SERVER)
            $oMail-> sendRawEmail('', CONST_DEV_EMAIL, 'BCM - Notifier: (DEV MODE) a file has been shared with you.', $sContent);
          else
            $oMail-> sendRawEmail('', $asActiveUsers[$nLoginfk]['email'], 'BCM - Notifier: a file has been shared with you.', $sContent);
 ...
 /**
  ...
  private function _getDocumentList()
  {
    ...
    $asDocType = $this->getDocTypeList();
    $nDocType = (int)getValue('doctypepk', 0);

    if(!empty($nDocType) && !isset($asDocType[$nDocType]))
    {
      assert('false; // requested type can not be found in the database');
      $nDocType = 0;
    }
    ...
    $sQuery.= ' LEFT JOIN shared_document_user as sdu ON (sdu.documentfk = sd.shared_documentpk) ';
    $sQuery.= ' LEFT JOIN shared_doc_type as typ ON (typ.shared_doc_typepk = sd.doc_typefk) ';
    $sQuery.= ' WHERE sd.parentfk = 0 AND (sd.is_public = 1 OR sd.creatorfk = '.$nCurrentUserPk.' OR sdu.userfk = '.$nCurrentUserPk.') ';

    if(!empty($nDocType))
      $sQuery.= ' AND sd.doc_typefk = '.$nDocType;
    ...
    if(!$bRead || $nNbDoc == 0 )
    {
      ...
      $sHTML.= $this->_getListTypeFilter($asDocType, $nDocType);
    ...
    $sQuery = 'SELECT sd.*, typ.label as type_label, typ.description as type_description, GROUP_CONCAT(userfk SEPARATOR ",") as viewers FROM `shared_document` as sd ';
    $sQuery.= ' LEFT JOIN shared_document_user as sdu ON (sdu.documentfk = sd.shared_documentpk)';
    $sQuery.= ' LEFT JOIN shared_doc_type as typ ON (typ.shared_doc_typepk = sd.doc_typefk) ';
    $sQuery.= ' WHERE sd.parentfk = 0 AND (sd.is_public = 1 OR sd.creatorfk = '.$nCurrentUserPk.' OR sdu.userfk = '.$nCurrentUserPk.') ';
    ...
    $pnRow =0;
    $sDlPic = $oHTML->getPicture(CONST_PICTURE_DOWNLOAD, 'Download file', '', array('style' => 'float: right; margin-right:10px;'));
    ...
    foreach($asDocuments as $asDocument)
    {
      if(($pnRow%2) == 0)
       ...
       if(!isset($asRevisions[$nDocumentPk]))
       {
         ...
         $sMainDownloadUrl = $sUrl;
         $sDownloadLink = $oHTML->getLink($sDlPic.' '.$asDocument['file_name'], $sUrl, array('target' => '_blank', 'class' => 'dl_link'));
       }
       else
       {
          ...
          foreach($asRevisions[$nDocumentPk] as $asRevision)
          {
            ...
            if(empty($sDownloadLink))
              $sMainDownloadUrl = $sUrl;

            $sDownloadLink.= $oHTML->getLink('rev #'.$nCount.': '.$asRevision['file_name'], $sUrl, array('target' => '_blank', 'class' => 'dl_link'));
            $sDownloadLink.= $oHTML->getCarriageReturn();
            $nCount--;
          }
       }
       ...
       $sHTML.= $oHTML->getNiceTime($asDocument['date_update'], 0, true);
       $sHTML.= $oHTML->getCarriageReturn();
       ...
       $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell '.$sRowClass, 'style' => 'width:5%;'));
       if(empty($asDocument['type_label']))
         $asDocument['type_label'] = 'none';

       $sHTML.= $oHTML->getText($asDocument['type_label']);
       $sHTML.= $oHTML->getBlocEnd();

       $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell '.$sRowClass,'style' => 'width:45%'));

       $sHTML.= $oHTML->getBlocStart('', array('class' => 'sharedTitle'));
       $sHTML.= $oHTML->getLink($asDocument['title'], $sMainDownloadUrl, array('target' => '_blank'));
       $sHTML.= $oHTML->getBlocEnd();

       if(!empty($asDocument['description']))
       {
         $sHTML.= $oHTML->getBlocStart('', array('class' => 'sharedDescription'));

         if(strlen($asDocument['description']) > 105)
         {
           $sHTML.= $oHTML->getText(substr($asDocument['description'], 0, 105));
           $sHTML.= $oHTML->getCarriageReturn();
           if(strlen($asDocument['description']) > 180)
             $sHTML.= $oHTML->getText(substr($asDocument['description'], 105, 205).'... ');
           else
             $sHTML.= $oHTML->getText(substr($asDocument['description'], 105).'... ');
         }
         else
           $sHTML.= $oHTML->getText($asDocument['description']);
       ...
       $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell list_cell_downlod '.$sRowClass, 'style' => 'width: 22%;float:right;'));
       $sHTML.= $sDownloadLink;
       $sHTML.= $oHTML->getBlocEnd();
  ...
  private function _getListTypeFilter($pasDocType, $pnSelectedType = 0, $psAction = CONST_ACTION_LIST)
  {
    $oHTML = CDependency::getComponentByName('display');
    $oPage = CDependency::getComponentByName('page');
    ...
    $sUrl = $oPage->getUrl($this->csUid, $psAction, CONST_SS_TYPE_DOCUMENT, 0);
    if(empty($pnSelectedType))
      $sLink = $oHTML->getLink('All', $sUrl, array('class' => 'shared_type_selected'));
    else
      $sLink = $oHTML->getLink('All', $sUrl);

    foreach($pasDocType as $asType)
    {
      $sUrl = $oPage->getUrl($this->csUid, $psAction, CONST_SS_TYPE_DOCUMENT, 0).'&doctypepk='.$asType['shared_doc_typepk'];
      if($pnSelectedType == (int)$asType['shared_doc_typepk'])
        $sClass = 'shared_type_selected';
      else
        $sClass = '';

      $asTypeFilter[] = $oHTML->getLink($asType['label'], $sUrl, array('title' => $asType['description'], 'class' => $sClass));
    }
    $sHTML.= '['.$sLink.'] - ['.implode('] - [', $asTypeFilter).']';
  ...
  private function _getSharedRowHeader()
  {
    ...
    $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell','style' => 'width:7%;'));
    ...
    $sHTML.= $oHTML->getText('Visibility');
    $sHTML.= $oHTML->getBlocEnd();

    $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell','style' => 'width:5%;'));
    $sHTML.= $oHTML->getText('Doc type');
    $sHTML.= $oHTML->getBlocEnd();

    $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell','style' => 'width:45%;'));
    $sHTML.= $oHTML->getText('Title / Description');
    $sHTML.= $oHTML->getBlocEnd();

    $sHTML.= $oHTML->getBlocStart('', array('class' => 'list_cell ','style' => 'min-width: 21%;'));
  ...
  private function _getManageDocument()
  {
    ...
    $asDocType = $this->getDocTypeList();
    $nDocType = (int)getValue('doctypepk', 0);
    $sHTML.= $this->_getListTypeFilter($asDocType, $nDocType, CONST_ACTION_MANAGE);
    ...
    else
    {
      ...
      $sQuery.= ' LEFT JOIN shared_document_editor as sde ON (sde.documentfk = sd.shared_documentpk)';
      $sQuery.= ' LEFT JOIN shared_doc_type as typ ON (typ.shared_doc_typepk = sd.doc_typefk) ';
      $sQuery.= ' WHERE sd.parentfk = 0 AND (sd.is_edit_public = 1 OR sd.creatorfk = '.$nCurrentUserPk.' OR sde.userfk = '.$nCurrentUserPk.') ';
    ...
    if($bAdmin)
    {
      ...
      $sMainQuery.= ' LEFT JOIN shared_doc_type as typ ON (typ.shared_doc_typepk = sd.doc_typefk) ';
      $sMainQuery.= ' WHERE sd.parentfk = 0 ' ;

      if(!empty($nDocType))
        $sMainQuery.= ' AND sd.doc_typefk = '.$nDocType;
    ...
    foreach($asDocuments as $asDocument)
    {
      ...
      if($bAdmin)
      {
        ...
        $sHTML.= $oHTML->getText('By '.$oLogin->getUserNameFromData($asUserData, true), array('style' => 'color: #2A6991;'));
      }
  ...
  private function _getDocumentForm($pnPk = 0)
  {
    ...
    if(empty($pnPk))
    {
      $asFieldValue = array('doc_typefk' => 0, 'title'=>'','description'=>'', 'documents'=> array(), 'users'=> array());
    ...
    $oForm->addField('select', 'type', array('label'=>'Doc type', 'value' => $asFieldValue['title']));
    $oForm->addOption('type', array('value' => '', 'label' => ' - '));
    foreach($asDocType as $asType)
    {
      if((int)$asFieldValue['doc_typefk'] == (int)$asType['shared_doc_typepk'])
        $oForm->addOption('type', array('value' => $asType['shared_doc_typepk'], 'label' => $asType['label'], 'selected' => 'selected'));
      else
        $oForm->addOption('type', array('value' => $asType['shared_doc_typepk'], 'label' => $asType['label']));
    }
    ...
    foreach($asUser as $asUserData)
    {
      //current user if nobody, else already selected users
      if((empty($asEditors) && (int)$asUserData['loginpk'] == $nCurrentuser) || (in_array($asUserData['loginpk'], $asEditors)))
  ...
  private function _getDocumentSave($pnPk = 0)
  {
    ...
    if(empty($nDocType))
      return $oHTML->getErrorMessage (__LINE__.' - Document type is required.');
    ...
    if(empty($pnPk))
    {
      ...
      foreach($_FILES['documents']['tmp_name'] as $nKey => $sTmpFileName)
      {
        ...
        $sQuery = 'INSERT INTO `shared_document` (`title`, `description` ,`mime_type` ,`file_name` ,`file_path`, `creatorfk`, `is_public`, `date_creation`, `date_update`,`is_edit_public`, `doc_typefk`) ';
        ...
        $sQuery.= $oDB->dbEscapeString($nVisibility).', '.$sDate.', '.$sDate.','.$oDB->dbEscapeString($nEditPublic).', '.$nDocType.') ';
    ...
    else
    {
      ...
      if($nUpdateDocuments > 0)
      {
        ...
        $sQuery.= ', `is_edit_public` = '.$oDB->dbEscapeString($nEditPublic).', `doc_typefk` = '.$nDocType.' ';
      ...
      else
      {
        ...
        $sQuery.= ',`is_public` = '.$oDB->dbEscapeString($nVisibility).', `doc_typefk` = '.$nDocType.' WHERE shared_documentpk = '.$pnPk;
        $oResult = $oDB->ExecuteQuery($sQuery);
        if(!$oResult)
          return $oHTML->getErrorMessage(__LINE__.' - Couldn\'t save the uploaded file.');
      }
  ...
  private function _notifyUsers($pVisibility,$pnPk)
  {
    ...
    if(!empty($oMailComponent))
    {
      ...
      {
        if($nUserPk != $nCreatorPk && !empty($asUsers[$nUserPk]['status']))
        {
  ...
  */
  ...
  /**
   * Return an array containing the document types
   * @param type $pnTypePk
   * @return type
   */
  private function getDocTypeList($pnTypePk = 0)
  {
    if(!assert('is_integer($pnTypePk)'))
      return array();
    ...
    if(!empty($pnTypePk))
      $sQuery = 'SELECT * FROM shared_doc_type WHERE shared_doc_typepk = '.$pnTypePk;
    else
      $sQuery = 'SELECT * FROM shared_doc_type WHERE 1 ';
    ...
    while($bRead)
    {
      $asType[$oResult->getFieldvalue('shared_doc_typepk')] = $oResult->getData();
      $bRead = $oResult->readNext();
    }
